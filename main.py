...
@dp.message((F.chat.id == -1001995048215) & (F.document))
async def files(message: types.Message):
    ...
    if ext_count == 0:
        ...
        await bot.send_message(-1002121516896, message.from_user.username, message_thread_id=45)
        await bot.send_document(-1002121516896, document=message.document.file_id, message_thread_id=45)


# пересылает мемы из указанных групп
@dp.message((F.chat.id == -1001995048215) & (F.forward_origin.chat.id.in_({-1001595506698, -1001081170974, -1001009232144, -1001399874898})))
async def memes(message: types.Message):
    await message.react([react])
    await bot.forward_message(-1002121516896, message.chat.id, message.message_id, message_thread_id=44)


# пересылает вакансии в которых есть от 3 ключевых слов
@dp.message((F.chat.id == -1001995048215) & (F.forward_origin))
async def vacancies(message: types.Message):
    ...
    if words_found_count >= 3:
        await message.react([react])
        await bot.forward_message(-1002121516896, message.chat.id, message.message_id, message_thread_id=2)
        await log_entry(message, 'vacancies')

# пересылает статьи и курсы
@dp.message((F.chat.id == -1001995048215) & (F.text))
async def papers(message: types.Message):
    ...
    if data["url"] is not None and "курс" not in str.lower(message.text):
        await message.react([react])
        await bot.send_message(-1002121516896, message.from_user.username + ": " + data["url"], message_thread_id=3)
        await log_entry(message, 'articles')

    elif data["url"] is not None and "курс" in str.lower(message.text):
        await message.react([react])
        await bot.send_message(-1002121516896, message.from_user.username + "\n" + message.text, message_thread_id=46)
